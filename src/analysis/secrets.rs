...
pub fn is_sensitive_filename(name: &str) -> bool {
    ...
    let sensitive_names = [
        ".env", "credentials", "secrets", "keystore", "id_rsa", "id_ed25519",
        "id_ecdsa", "id_dsa", "known_hosts",
    ];
    let sensitive_exts = [".pem", ".key", ".p12", ".pfx", ".jks", ".keystore", ".cer", ".crt"];
...
fn build_patterns() -> Vec<CompiledPattern> {
    ...
    let defs: &[(&'static str, &str)] = &[
        ...
        // === Communication ===
        ("Slack Token",             r"xox[bpraos]-[0-9]{10,12}-[0-9]{10,12}-[a-zA-Z0-9]{24}"),
        ...
        // === Database ===
        ("Connection String",       r"(?i)(postgres(ql)?|mysql|mongodb(\+srv)?|redis|amqp|mssql|sqlserver)://[^\s]{10,}"),

        // === Generic credential patterns ===
        ("Password in URL",         r"://.{1,64}:.{8,}@[^\s]+"),
        ("Generic API Key",         r"(?i)(?:api[_\-]?key|apikey|api[_\-]?secret|access[_\-]?key)\s*[=:]\s*[A-Za-z0-9_\-\.]{16,}"),
        ("Env File Secret",         r"(?i)^(ANTHROPIC_API_KEY|OPENAI_API_KEY|OPENAI_KEY|GEMINI_API_KEY|DATABASE_URL|DB_PASSWORD|SECRET_KEY|AUTH_SECRET|JWT_SECRET|GITHUB_TOKEN|STRIPE_SECRET|AWS_ACCESS_KEY_ID|AWS_SECRET_ACCESS_KEY|TWILIO_AUTH_TOKEN|SENDGRID_API_KEY|CLOUDINARY_SECRET)\s*=\s*\S{8,}"),
    ];
...
mod tests {
    ...
    fn anthropic_key_detected() {
        ...
        assert!(!matches.is_empty(), "should detect Anthropic key, got: {:?}", matches);
    }
    ...
    fn openai_key_detected() {
        ...
        let matches = scan_text(text);
        assert!(!matches.is_empty(), "should detect OpenAI key, got: {:?}", matches);
    }
    ...
    fn no_false_positive_on_normal_text() {
        ...
        let matches = scan_text(text);
        assert!(matches.is_empty(), "should not flag normal text: {:?}", matches);
    }
}
